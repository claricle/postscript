...
RSpec.describe Postscript::Color do
  ...
  describe ".cmyk" do
    it "converts CMYK to RGB" do
      c = described_class.cmyk(0, 0, 0, 0)
      expect(c).to eq(Postscript::Color::WHITE)

      c2 = described_class.cmyk(1, 1, 1, 0)
  ...
  describe ".parse" do
    it "parses #rrggbb" do
      expect(described_class.parse("#aabbcc").to_svg).to eq("#aabbcc")
    end

    it "parses #rgb (3-digit)" do
      expect(described_class.parse("#abc").to_svg).to eq("#aabbcc")
    end

    it "parses named colors" do
      expect(described_class.parse("red")).to eq(described_class.rgb(1, 0,
                                                                     0))
    end

    it "returns nil for none" do
      expect(described_class.parse("none")).to be_nil
    end
  end

  describe "#to_ps_rgb" do
    it "emits space-separated [0,1] floats" do
      c = described_class.rgb(1, 0, 0)
  ...
  describe "#to_svg" do
    it "emits #rrggbb" do
      expect(described_class.rgb(0.5, 0.5, 0.5).to_svg).to eq("#808080")
    end
  end

  describe "value equality" do
    it "treats equal colors as ==" do
      expect(described_class.rgb(1, 0,
                                 0)).to eq(described_class.rgb(1, 0, 0))
    end
  end
end
